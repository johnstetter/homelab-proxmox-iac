...
# Complete pipeline: generate ISO and deploy template to Proxmox in one command
...
generate_iso() {
    ...
    if "$SCRIPT_DIR/generate-nixos-iso.sh" "${iso_args[@]}"; then
...
deploy_template() {
    ...
    if "$SCRIPT_DIR/create-proxmox-template.sh" "${template_args[@]}"; then
